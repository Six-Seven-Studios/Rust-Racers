        ...
        GameState::Title => {
            ...
            if is_typing_ip {
                for key in input.get_just_pressed() {
                    if let Ok(mut text) = server_ip_query.single_mut() {
                        match key {
                            KeyCode::Tab => {
...
pub fn check_for_lobby_input(
    ...
    mut server_ip_query: Query<&mut Text2d, (With<ServerIpInput>, Without<LobbyNameInput>)>,
    server_address: Res<ServerAddress>,
    buttons: Query<(&Interaction, &JoinButton), (Changed<Interaction>, With<Button>)>,
) {
    match *current_state.get() {
        GameState::Lobby => {
            if input.just_pressed(KeyCode::Escape){
        ...
        GameState::Creating => {
            ...
            for key in input.get_just_pressed() {
                if let Ok(mut text) = lobby_name_query.single_mut() {
                    match key {
                        KeyCode::Backspace => {
                            text.0.pop();
                        }
            ...
            if input.just_pressed(KeyCode::Escape){
                ...
                let lobby_name = if let Ok(text) = lobby_name_query.single() {
